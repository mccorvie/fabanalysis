...
Cutting to the chase, here are the results of the model.  In subsequent sections we will discuss the various methodologies and assumptions which comprise this approach.
...
With these prices, we translate revenues in each node category to quantities of wafers sold.  Summing over the quantities in each category yields a total number of wafers sold which can be compared to the published total number of wafers sold.  Our ASP estimates are primarily from the source published in 2022, and the total implied wafer production equals the reported wafer production within 5% for all four quarters of 2022.

A comparison between the estimated total wafers sold and the actual reported total wafers sold is shown below.  Using these prices underestimates production in periods earler than 2022, suggesting ASP's were lower then, and overestimates recent production, suggesting ASP's are higher.  Also plotted are the results of an adjustment described in the next section.
...
To account for broad price trends, we incorporate the Bureau of Labor Statistics' Producer Price Index by Industry.  Two potentially relevant series are [PCU33443344](https://fred.stlouisfed.org/series/PCU33443344) for "semiconductor and other electronic component manufacturing" and [PCU334413334413A](https://fred.stlouisfed.org/series/PCU334413334413A) for "other semiconductor devices, including transistors, diodes, and semiconductor parts such as wafers".  Both are plotted below, normalized to Q3 2022, the date of the article estimating ASP's above.  They show broadly similar trends, though the movements are larger for the more specific series ending in "13A".  

For each time period deflate baseline prices estimated in the previous section report by the PPI, normalized so that 3Q22 has a deflator of 1.  This adjustment makes the total output estimated from revenues and ASP's better match the actual total output.
...
The black points are the ASP's used in this analysis, and the purple are the error bars estimated from regression.  Note however that some of the error bars for the reciprocal ASP's pass through zero, which implies the error range for the ASP's includes infinity. 
...
## Capacity Model: Scanner Supply Chain
...
There are a number of critical inputs to wafer production: unprocessed silicon wafers, electricity, water, specialized gasses and other components.  However, one of the rate-limiting tools for manufacturing is the scanner, which uses lasers to imprint the design of the semiconductor on the photoresist.  Modern nodes use extreme ultraviolet (EUV) scanners.  ASML is a dutch company which manufactures lithography scanners.  Importantly, is the only global supplier for the EUV lithography scanners used in advanced nodes.
...
Primarily using ASML earnings reports, we estimate the specific scanner product lines sold to TSMC, and use published specs for each product line to estimate scanner throughput.  
...
The specs for each model include the throughput measured in "wafers per hour" (wph) at 100% utilization.  One important factor in the manufacturing of advanced nodes is the marked increase in the number of exposures required to manufacture a chip.  The manufacturing process may result in multiple physical layers or multiple exposures of related masks to allow for finer pitch.  In terms of throughput, we need an estimate for the typical number of exposures for a finished product, and industry shorthand which encompasses a number of different manufacturing processes is the number of "layers".

More advanced nodes have seen a big increase in the number layers.  The number of layers depends on the particular semiconductor being produced and is not universal to a node.  Logic semiconductors tend to require more layers than memory semiconductors.  Some sources distinguish manufacturing processes into finer categories, such as "N3" as compared with "N3E".  In any case, the trend toward increasing numbers of scans for advanced nodes is an important factor in manufacturing capacity, and we make some simplifying assumptions listed below.
...
Below is an estimate of the ASP of particular scanner models. There are a lot of sources of uncertainty in these estimates.  First, public reports about scanner prices are sketchy and approximate and rarely mention specific models.  Almost certainly, product lines and their prices have changed over time.  As mentioned previously, ASP's for TSMC are likely higher than the overall reported ASP as they are early adopters of scanner technology.
...
We refer to two sources for the power usage of particular scanner models.  The first is on page 68 [ASML's 2020 annual report](https://www.annualreports.com/HostedData/AnnualReportArchive/a/NASDAQ_ASML_2020.pdf) which suggests that at 100% untilization, the NXE:3400C consumes 1.31MW.  While we don't have exact figures for the NXE:3800D, [Jones 2023](https://ieeexplore.ieee.org/document/10413715) and media reports suggest its about the same accounting for increased complexity of more advanced manufacturing processes and increased efficiency of later models.  The actual energy usage depends on unknown variables such as the energy dose of each exposure and the number of steps in the manufacturing process.

Assuming each phase consumes 200MW of power, scanners account for 11% of that usage, and and each scanner consumes 80% x 1.31MW, the eight fab phases at TSMC Fab 18 support 167 scanners.  This is consistent with the analysis above from ASML earnings reports.  
...
Many aspects of this analysis could be refined or improved, especially with access to superior data sources.  However, the relative consistency of the results of the two methodologies is lends credence to the results of both approaches.
